# /secrets — Secrets & Credential Scan

**Input:** the target you type after the command (a file, directory, or `.`); default to `.` if empty.

Hunt for exposed secrets. The most common MVP breach: a real key committed to git or bundled into client-side code. Follow the operating rules in `.cursor/rules/operating-rules.mdc`. **Do not print full secret values — redact to first/last 4 chars.**

## Where to look

### 1. Source & config files
High-signal patterns: `sk_live_`, `sk_test_`, `pk_live_`, `rk_live_` (Stripe); `AKIA`, `ASIA` (AWS); `AIza` (Google); `ghp_`, `gho_`, `github_pat_` (GitHub); `xox[baprs]-` (Slack); long `eyJ` JWTs; `-----BEGIN * PRIVATE KEY-----`; Supabase `service_role` / `SUPABASE_SERVICE_ROLE_KEY`; `sk-`, `sk-ant-` (OpenAI/Anthropic); generic `password`/`secret`/`api_key`/`token`/`client_secret` with a non-placeholder value.

### 2. Tracked secret files
```
git ls-files | grep -Ei '\.env($|\.)|\.pem$|\.key$|\.pfx$|credential|secret'
```
Any real `.env` (not `.env.example`) under version control is a finding.

### 3. Git history
Secrets removed from HEAD but still in history are still leaked:
```
git log --all -p -S 'sk_live_' -- ':!*.md' | head
```
Repeat for the key prefixes above. Rotation is required — deleting the file does not un-leak it.

### 4. Client bundle exposure (critical for web MVPs)
- Secrets behind a public prefix that ships to the browser: `NEXT_PUBLIC_*`, `VITE_*`, `REACT_APP_*`, `PUBLIC_*` holding more than a publishable key.
- Secrets hardcoded in frontend `.js`/`.ts`/`.tsx`.
- `service_role` / server-only keys referenced anywhere reachable by the client.

## Report

| Secret type | Location (file:line / git rev) | Exposure | Severity | Action |
|-------------|-------------------------------|----------|----------|--------|

- **Exposure**: source-only / in git history / shipped to client.
- **Action**: always **rotate the key** (assume compromised), then remove from code/history and move to a secret manager or server-only env var.

If a secret is confirmed shipped to the client or present in public git history, mark **Critical** and recommend rotating before anything else.
